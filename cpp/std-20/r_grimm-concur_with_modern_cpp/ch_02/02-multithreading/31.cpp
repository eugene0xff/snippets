...
void function1(std::stop_token stopToken, std::string const& str) {
...
int main() {
	...
	std::thread t1 = std::thread(function1, stopToken, "std::thread");

	std::jthread jt = std::jthread(function1, stopToken, "std::jthread");
	...
	std::thread t2(function2, std::move(prom), stopToken, "std::promise");
